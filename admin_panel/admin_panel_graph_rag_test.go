...
func TestGraphRAGPreviewUIIncludesNavigationAndDocumentLoading(t *testing.T) {
	...
	for _, want := range []string{
		...
		`id="graph-fit"`,
		`id="graph-zoom-level"`,
		`id="graph-concept-node-limit"`,
		`id="graph-concept-relationship-limit"`,
		`id="graph-apply-concept-limits"`,
		`id="graph-category-filter"`,
		`id="graph-category-options"`,
		`id="graph-category-show-all"`,
		`id="graph-category-hide-all"`,
		`id="graph-export-png"`,
