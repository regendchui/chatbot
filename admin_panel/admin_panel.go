...
func registerAdminPanelRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/admin/table/conversation/engagement/exclude", adminRequireAuth(adminEngagementExcludeHandler))
	...
	mux.HandleFunc("/admin/graph-rag", adminRequireAuth(adminGraphRAGHandler))
	mux.HandleFunc("/admin/graph-rag/select", adminRequireAuth(adminGraphRAGSelectHandler))
	mux.HandleFunc("/admin/graph-rag/rebuild", adminRequireAuth(adminGraphRAGRebuildHandler))
	mux.HandleFunc("/admin/graph-rag/rebuild-stale", adminRequireAuth(adminGraphRAGRebuildStaleHandler))
	...
	mux.HandleFunc("/admin/graph-rag/complete-preview", adminRequireAuth(adminGraphRAGCompletePreviewHandler))
	...
	mux.HandleFunc("/admin/intention-routing-rag/validate", adminRequireAuth(adminIntentionRoutingRAGValidateHandler))
	mux.HandleFunc("/admin/intention-routing-rag/publish", adminRequireAuth(adminIntentionRoutingRAGPublishHandler))
	mux.HandleFunc("/admin/intention-routing-rag/discard-draft", adminRequireAuth(adminIntentionRoutingRAGDiscardDraftHandler))
	mux.HandleFunc("/admin/intention-routing-rag/test", adminRequireAuth(adminIntentionRoutingRAGTestHandler))
	...
	mux.HandleFunc("/admin/client-info/messages", adminRequireAuth(adminClientInfoMessagesHandler))
